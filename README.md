# Evosint v2.18 — "Ghost" Monochrome Console (fictional agency theme)
...
| Kitty | **Clicker arcade: 1,000 taps on the 🐈‍⬛ = +20 scans** — tap-evidence protocol (timing + pointer trail per tap), rhythm forensics (metronome / fixed-point / impossible-rate / forged-clock detection), suspicion scoring with Turnstile-or-cooldown fair-play challenge, held-not-lost awards, arrival token bucket, 10 awards/day |
| World | **Live planet: zone weather + ticking local clock, local news + conflict wire (Google News RSS, GDELT fallback), global attack telemetry (live Feodo C2s + fresh CISA KEV)** — **interactive dot-matrix globe** (drag/inertia, tap-to-pick, day-night shading) plus **Streets 3D**: self-hosted MapLibre GL v5 + free OpenFreeMap dark tiles, true globe projection, 3D buildings from zoom 14, tap-to-probe, loading veil + auto-fallback if tiles are blocked, choice remembered |
